...
async fn run_ios_pipeline(
    ...
    // Bundle project assets into .app so the app can find images at runtime
    let assets_src = project_dir.join("assets");
    if assets_src.is_dir() {
        let assets_dest = app_path.join("assets");
...
async fn run_macos_pipeline(
    ...
    // Bundle project assets into Contents/Resources/ so the app can find
    // images/files at runtime via NSBundle.mainBundle.resourcePath
    let assets_src = project_dir.join("assets");
    if assets_src.is_dir() {
